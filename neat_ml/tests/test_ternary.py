...
data = importlib.resources.files("neat_ml").joinpath(
    "data/Hypothetical_ternary_phase_map.xlsx"
)


@pytest.mark.parametrize(
    "X, y, clockwise, expected",
    [
        ...
        # for us)
        (
            np.array([[50, 20, 30], [10, 60, 30], [10, 30, 60]]),
            np.array([0, 0, 1]),
            True,
            [
        ...
        (
            pd.read_excel(data, sheet_name=[0], header=1)[0]
            .iloc[..., 4:-1]
        ...
        (
            pd.read_excel(data, sheet_name=[1], header=1)[1]
            .iloc[..., 4:-1]
            .to_numpy(),  # type: ignore
            np.ones(43),
            False,
...
def test_ternary_phase_diagram(tmp_path, X, y, clockwise, expected):
    ...
    actual_fig = lib.plot_tri_phase_diagram(
        X,
        y,
        plot_path=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            np.array([[50, 20, 30], [10, 60, 30], [10, 30, 59]]),
            np.zeros(3),
            "do not sum",
        ),
        ...
        # a constant value
        (
            np.array([[50, 20, 30, 2], [10, 60, 30, 2], [10, 30, 60, 2]]),
            np.zeros(3),
            "three variables",
        ),
    ],
